...
% The model is populated by a unit continuum of infinitely lived households indexed $i \in [0, 1]$. Each period, households provide labor $\ell_{i, t}$ decided by the union and choose to consume $c_{i, t}$ and hold $b_{i, t}$ of a risk-free government bond which has gross real returns $R_t$ to maximize expected discounted utility. Households have constant relative risk aversion (CRRA) preferences given by 
% \begin{equation*}
...
% Because productivity $z_{i, t}$ follows and exogenous law of motion that is time invariant, the distribution of household productivity $\Gamma_t^z (z)$ is also fully exogenous and follows a time invariant process. Assuming the initial distribution for $\Gamma_t^z (z)$ is equal to the ergodic distribution of the AR-1 process, the overall distribution stays constant over time.
...
% \end{equation*}
% The union chooses $\ell_k$ to maximize household utility subject to quadratic adjustment $m_{k, t}^W$ costs
% \begin{equation*}
...
% This union block follows \textcite{auclert2023mpcs}. The assumption that unions demand labor uniformly from households regardless of their productivity and wealth ignores differences in household willingness to work and thereby means some households will be required to work more than they would choose to \parencite{gerke2024household}. Alternative approaches would vary the amount of labor demanded or wage for households to deal with this, but add substantial mathematical and computational complexity to the model \parencite{gerke2024household}.
...
% Therefore, a solution to the model consists of sequences for prices $\{\pi_t, W_t, \pi_t^W, M_t, D_t, R_t, I_t, \tau_t^L\}_{t = 0}^\infty$, household decision rules $\{b_t, c_t\}_{t = 0}^\infty$ that solve the household utility maximization problem, the distribution of household states $\{\Gamma_t\}_{t = 0}^\infty$ that evolves following the policy rules, and macroeconomic aggregates $\{Y_t, N_t, L_t, B_t, G_t\}_{t = 0}^\infty$ all so that the labor, bond, and goods markets clear subject to exogenous, AR(1) processes for $\{A_t, g_t, \xi_t, \psi_t, \tau_t^P, \eta_t\}_{t = 0}^\infty$.
